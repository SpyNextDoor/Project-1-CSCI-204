...
# ```
# The music score specifies how many samples the song has (**num_of_samples**) and how many instruments are used (**num_of_instruments**).
# For each instrument, it specifies which wave type (**wave_type_x**) should be used to generate the wave sound, which envelope should be applied (**envelope_type_x**),
# the instrument amplitude (**instrument_amplitude_x**), and the pan angle (**pan_angle_x**). Then, it describes all the notes of a song.
# Each note has an instrument index (**note_x_instrument_index**), its own amplitude (**note_x_amplitude**), and from which sample (**note_x_start_sample_index**)
# to which sample (**note_x_end_sample_index**) that the note sounds in the song. For example, below is a simple do-re-mi-fa-so melody described using the above music score format:
# ```text
# 441001
# 1
# 1 2 0.8 0.02
...
#
# To generate a song from this music score, we need to create audio samples for each instrument.
# Then, for each note, we generate the corresponding sound wave, apply the envelope if needed, and accumulate the sound wave into the audio samples.
# After processing all the notes for each instrument and at each time position, we compute the averaged value.
# For example, if there are five notes accumulated to the i-th sample, then the i-th sample should be divided by five.
# At least, for each instrument, we mix its audio data into stereo song data by using its own amplitude and pan angle.
#
...
# Your task is to implement the Song.Song class that inherits from Wave.BaseWave. The initializer of this class should take one parameter -- the simple formatted music sheet, and use it to render a song accordingly.
# To break down the song rendering logic, we recommend the below helper methods:
...
# ```
# However, you are free to implement the Song.Song class in whatever way you want. If you don't like our recommendation,
# feel free to remove all the methods and implement your own Song.Song class as you desire. As long as it produces the same song, you will obtain the full score.
#
...
class Song(BaseWave):
    ...
    def _read_instrument_info(self, in_file: TextIO, instrument_info: List[Dict]) -> None:
        ...
        num_instruments = len(instrument_info)
        for inst in range(num_instruments):
            a = in_file.readline()
            line = a.split()
            instrument_info[inst]['wavetype'] = int(line[0])
            instrument_info[inst]['envelope'] = int(line[1])
            instrument_info[inst]['amplitude'] = float(line[2])
            instrument_info[inst]['pan'] = float(line[3])
    ...
    def _generate_instrument_note_wave(self, wave_type: int, num_samples: int, freq: float, amp: float) -> BaseWave:
        ...
        """
        if wave_type == 1:
            sound_wave = SineWave(num_samples, freq, amp)
        elif wave_type == 2:
            sound_wave = SquareWave(num_samples, freq, amp)
        elif wave_type == 3:
            sound_wave = SawtoothWave(num_samples, freq, amp)
        elif wave_type == 4:
            sound_wave = ComplexWave(num_samples, freq, amp)
        else:
            sound_wave = StringWave(num_samples, freq, amp)
        return sound_wave._data
    ...
    def _read_instrument_audio_data(self, in_file: TextIO, instrument_info: List[Dict], audio_data: List[Array], audio_num_samples: List[Array]):
        ...
        This method reads **in_file** line by line until the end. Each line represents a note of an instrument.
        After splitting the line into values, this method should obtain the instrument index (telling us which instrument the note belongs to),
        the note number (which can be converted to frequency using audio_note_number_to_freq()), the note amplitude, the starting and ending sample indices.
        Using this information, this method calls _generate_instrument_note_wave() to create the note sound wave,
        then applies the envelope, and at last accumulates the samples in **audio_data** (using the instrument index,
        the starting and ending sample indices). This method also increases the
        number of sample counters **audio_num_samples** whenever it accumulates a sound sample to **audio_data**.
        ...
        for line in in_file:
            ...
            inst_idx = int(line_list[0])
            note_num = int(line_list[1])
            freq = audio_note_number_to_freq(note_num)
            note_amp = float(line_list[2])
            start_sample = int(line_list[3])
            end_sample = int(line_list[4])
            num_samples = end_sample - start_sample
            wave_type = instrument_info[inst_idx]['wavetype']
            note_wave = self._generate_instrument_note_wave(wave_type, num_samples, freq, note_amp)

            if instrument_info[inst_idx]['envelope'] == 1:
                audio_rise_fall_envelope(note_wave)
            elif instrument_info[inst_idx]['envelope'] == 2:
                audio_adsr_envelope(note_wave)

            counter = 0
            for i in range(start_sample, end_sample):
                audio_data[inst_idx][i] = note_wave[counter]
                audio_num_samples[inst_idx][i] += 1
                counter += 1

            print(audio_num_samples)
    ...
    def _average_audio_data_samples(self, audio_data: List[Array], audio_num_samples: List[Array]):
        ...
        This method computes the average sample values. Note that **audio_data** stores the accumulated sample values while **audio_num_samples** stores the total number of samples.
        To compute the average values, this method loops each instrument and loops each sample data in **audio_data**, then divides the sample value by the total number of samples.
        Note that this method only divides the sample value by the total number of samples when the total number of samples is not zero.
        ...
        print(audio_length)
        samples_num = len(audio_data[0])
        print(samples_num)
        for sample_idx in range(samples_num):
            idx_i = 0
            for ins in range(audio_length):
                idx_i += audio_num_samples[ins][sample_idx]
            if idx_i > 1:
                for inst in range(audio_length):
                    audio_data[inst][sample_idx] /= idx_i
    ...
    def _mix_instrument_audio_in_song(self, instrument_info: List[Dict], audio_data: List[Array]):
        ...
        ins_amt = len(audio_data)
        number_of_samples = len(audio_data[0])
        audio_data_left = [Array(number_of_samples, 0) for _ in range(ins_amt)]
        audio_data_right = [Array(number_of_samples, 0) for _ in range(ins_amt)]
        for ins in range(ins_amt):
            gains = audio_stereo_gains(instrument_info[ins]['pan'])
            ins_amp = instrument_info[ins]['amplitude']
            gain_left = ins_amp * gains[0]
            gain_right = ins_amp * gains[1]
            for i in range(number_of_samples):
                audio_data_left[ins][i] = gain_left * audio_data[ins][i]
                audio_data_right[ins][i] = gain_right * audio_data[ins][i]
            audio_stereo_mix_in(self._data, audio_data_left[ins], 0)
            audio_stereo_mix_in(self._data, audio_data_right[ins], 1)
